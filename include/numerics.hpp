...
namespace infra
{
    ...
    template<class T> constexpr inline std::enable_if_t<infra::is_complex_v<T>, T> conj (T n){return std::conj(n);} 
    template<class T, size_t N> constexpr inline std::array<T, N> conj(std::array<T, N> vec){
        if constexpr(infra::is_complex_v<T>) for(auto& n : vec) n = std::conj(n);
        return vec;
    }
    ...
    template<class T, size_t N> constexpr inline T operator | (vec<T, N> row_major_matrix_or_vec, T col_major_vec_or_scalar)
    {
        ...
        if constexpr (std::is_arithmetic_v<T> || infra::is_complex_v<T>)
        {
